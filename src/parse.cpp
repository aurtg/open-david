...
formatter_t argument = (startswith(alpha | is('*')) & many(ascii)) | many(digit) | quotation;
formatter_t parameter = many(ascii | is(':'));
formatter_t name = many(general | is(':'));
formatter_t predicate = many(general);
